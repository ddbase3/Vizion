...
final class PieChartReportExporter extends AbstractReportExporter {
	...
	public function toString(): string {
		...
		if(count($result->columns) < 2) {
			throw new \RuntimeException('Pie chart export requires at least two result columns.');
		}
		...
		return '<div style="height:300px;"><canvas id="' . $uniqueId . '"></canvas></div>'
			...
			. 'var ctx=document.getElementById(' . json_encode($uniqueId) . ');'
			. 'new Chart(ctx,{type:"doughnut",options:{responsive:true,maintainAspectRatio:false},data:{labels:labels,datasets:[{label:' . $labelJson . ',data:data,hoverOffset:4}]}});'
			. '})();</script>';
	}
