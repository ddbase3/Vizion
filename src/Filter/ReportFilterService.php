...
final class ReportFilterService implements IReportFilterService {
	...
	private function buildGridFilterField(array $field, array $definition): array {
		...
		foreach(['minWidth', 'maxWidth', 'min', 'max', 'step', 'control', 'source', 'emptyValue'] as $key) {
	...
	private function normalizeValue(mixed $value, array $definition): mixed {
		...
		return match($type) {
			...
			'daterange', 'datetimerange' => $this->normalizeDateRangeValue($value),
	...
	private function normalizeDateRangeValue(mixed $value): array {
		...
		return [
			'from' => $this->normalizeScalarTextValue($value['from'] ?? ''),
			'to' => $this->normalizeScalarTextValue($value['to'] ?? '')
		];
	}
